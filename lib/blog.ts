...
export function getPosts(): PostMeta[] { return blogRegistry.map(({content, ...m}) => m); }
export function getPost(slug: string): PostCompleto | null { const p = blogRegistry.find(p => p.slug === slug); return p || null; }
